...
def get_extra_context(self, request, instance):
    ...
    # Prepare a set to store the names of fields that are related tables
    related_table_field_names = set()
    ...
    for rel in instance._meta.get_fields():
        if rel.is_relation and rel.auto_created and not rel.concrete:
            ...
            if related_objects.exists():
                # Add the field name to the set of related table field names
                related_table_field_names.add(rel.get_accessor_name())
    ...
    # Now process the regular fields, skipping the ones in related_table_field_names
    for field in instance._meta.get_fields():
        # Skip excluded fields and those that are related tables
        if field.name in excluded_extras or field.name in related_table_field_names or ('Requirement' in field.name):
            continue
